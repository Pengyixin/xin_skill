...
```python
from gerrit_client import GerritFetcher
import json
...
# 获取 diff
commit_msg, diff = fetcher.fetch_commit_and_diff("https://scgit.amlogic.com/#/c/642117/")
...
for url in change_urls:
    try:
        commit_msg, diff = fetcher.fetch_commit_and_diff(url)
        change_id = fetcher.extract_change_id(url)
...
#### 构造函数
```python
GerritFetcher(base_url: str, username: str, password: str)
```
...
**fetch_commit_and_diff(gerrit_url: str) -> Tuple[str, str]**
- 获取 commit message 和 diff 内容
- 返回：(commit_message, diff_content)

**extract_diff_content(patch_text: str) -> str**
- 从 patch 文本中提取 diff 部分

### GerritClient 类

扩展功能类，提供更完整的 Gerrit 操作：
- `get_change_by_id(change_id)` - 通过 ID 获取 change 信息
- `get_change_by_url(url)` - 通过 URL 获取 change 信息
- `is_change_merged(identifier)` - 检查 change 是否已合并
- `search_changes(query)` - 搜索 changes
- `batch_check_merged(urls)` - 批量检查合并状态
